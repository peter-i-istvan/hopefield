# Source: https://github.com/ItsWajdy/Hopfield_Network
import os.path
import numpy as np
...
def solve_hopefield(test_case: TestCase) -> np.array:
    hp = Hopfield(test_case.cities, test_case.distances, 50.0)
    return hp.predict(A=100.0, B=100.0, C=90.0, D=100.0, sigma=1, max_iterations=1000)


def main():
    test_case = read_test_case(os.path.join('test_cases', '15C0.txt'))
    v = solve_hopefield(test_case)
    print(v)
